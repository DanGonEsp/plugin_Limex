...
static void DomainAlgebra(TRegistry& reg, string grp)
{
...

	{
			...
			reg.template add_class_<T,TBase,TDebugBase>(name, grp)
			  ...
			  .add_method("get_step", &T::get_step)
			  .add_method("get_total_steps", &T::get_total_steps)
			  .add_method("get_success_steps", &T::get_success_steps)
			  .add_method("get_failed_steps", &T::get_failed_steps)
